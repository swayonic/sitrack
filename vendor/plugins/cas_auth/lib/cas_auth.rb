...
module CAS
  ...
  class Filter
      ...
      def filter_r(controller)
        ...
        if receipt
          ...
          if reqticket
            ...
            logger.info("will send redirect #{guess_service(controller)}") if valid
            controller.send :redirect_to, guess_service(controller) if valid
            return false # This is so that the filter will return false
                  # (even though it is valid) because we're already redirecting
                  # and we don't want code after the filter to redirect again
          else
    ...
    def self.redirect_url(controller)
      "#{@@login_url}?service=#{service_url(controller)}" + ((@@renew)? "&renew=true":"") + ((@@gateway)? "&gateway=true":"") + ((@@query_string.nil?)? "" : "&"+(@@query_string.collect { |k,v| "#{k}=#{v}"}.join("&")))
    end
    def self.guess_service(controller)
      req = controller.request
      parms = controller.params.dup
      parms.delete("ticket")
      query = (parms.reject {|key, val| key =~ /^action|controller$/}.collect {|key, val| "#{key}=#{val}"}).join("&")
      query = "?" + query unless query.empty?
      @@server_name ||= req.host_with_port()
