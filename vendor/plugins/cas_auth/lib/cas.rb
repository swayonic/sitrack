...
module CAS
  ...
  class Receipt
    attr_accessor :validate_url, :pgt_iou, :primary_authentication, :proxy_callback_url, :proxy_list, :user_name, :attributes
    ...
    def initialize(ptv)
      ...
      self.attributes = ptv.attributes
  ...
  class ServiceTicketValidator
    ...
    attr_reader   :pgt_iou, :user, :error_code, :error_message, :entire_response, :successful_authentication, :attributes
...
#      puts "successful"
      ...
      attributes.elements.each do |element|
        @attributes[element.name.to_sym] = element.has_text? ? element.get_text.value : nil
      end if attributes
#      puts "user: #{@user}"
