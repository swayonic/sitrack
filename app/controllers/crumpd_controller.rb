class CrumpdController < ApplicationController
	# Skip authentication
	skip_before_filter CASClient::Frameworks::Rails3::Filter, AuthenticationFilter, :authorize
	
	# POST /crumpd/dump
	def dump
		#TODO: Any way to authenticate?
	...
	def region_query
		ActiveRecord::Base.silence_auto_explain do
	  	# no automatic EXPLAIN is triggered here
			return SitrackTracking.find_by_sql('SELECT caringRegion FROM sitrack_tracking WHERE caringRegion IS NOT NULL GROUP BY caringRegion').map{|s| s[:caringRegion]}
		end
	end
