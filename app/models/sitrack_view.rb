...
class SitrackView < ActiveRecord::Base
  ...
  def display_columns
    out =  Person.table_name+'.fb_uid, '+Person.table_name+'.personID as id, '+HrSiApplication.table_name+'.applicationID, '
    ...
    if sitrack_view_columns.size > 0 
      ...
      sitrack_view_columns.each do |vc|
        ...
        if column.column_type == "project"
          out += '(SELECT MIN(name) FROM '+HrSiProject.table_name+' WHERE '+HrSiProject.table_name+'.SIProjectID = '+HrSiApplication.table_name+"."+column.select_clause+") as #{column.safe_name}"
        else
  ...
  def self.join_tables
    ...
    out += " LEFT JOIN #{SitrackTracking.table_name} on #{HrSiApplication.table_name}.applicationID = #{SitrackTracking.table_name}.application_id "
  	...
  	out += " LEFT JOIN #{HrSiProject.table_name} on #{HrSiApplication.table_name}.locationA = #{HrSiProject.table_name}.SIProjectID "
    return out
  end
